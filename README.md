<div align="center">

# Hi, I'm Sulaxmi Raskar 👋

### AI Engineer | Full Stack | RAG · LangChain · FastAPI · Next.js | Open to Work

[![LinkedIn](https://img.shields.io/badge/LinkedIn-sulaxmi--raskar-0A66C2?style=flat&logo=linkedin)](https://linkedin.com/in/sulaxmi-raskar)
[![Email](https://img.shields.io/badge/Email-sulaxmiraskar496%40gmail.com-EA4335?style=flat&logo=gmail)](mailto:sulaxmiraskar496@gmail.com)

</div>

---

## About Me

AI Engineer with **5+ years** of full-stack engineering experience building production-grade LLM-powered applications. MS Computer Science, 4.0 GPA · Cal State LA. Currently at **AT&T** building systems serving **10M+ daily users**.

I specialize in **RAG pipelines**, **agentic workflows**, and **LLM integration** — turning AI prototypes into deployed, scalable products using LangChain, LangGraph, FastAPI, and OpenAI APIs.

---

## Featured Projects

| Project | Description | Stack |
|---|---|---|
| 📚 [Smart Documentation Agent](https://github.com/sulaxmi22/smart-documentation-rag-agent) | Production-ready Q&A agent over custom documentation | NVIDIA LLMs · LlamaIndex · ChromaDB · FastAPI |
| 🤖 [RAG Resume Analyzer](https://github.com/sulaxmi22/RAG-Chatbot-LLM) | AI assistant that parses resumes and answers questions via RAG | LangChain · OpenAI · ChromaDB · FastAPI |
| 🌐 [Agentic Web Scraper](https://github.com/sulaxmi22/Agentic-Webscrapper) | Autonomous agent that scrapes, summarizes, and structures web data | LangGraph · OpenAI · Python |

---

## Tech Stack
```
AI / LLM   │ OpenAI API · LangChain · LangGraph · RAG · pgvector · ChromaDB · LlamaIndex · NVIDIA NIM
Backend    │ FastAPI · Python · Node.js · REST APIs · PostgreSQL · MongoDB
Frontend   │ React · Next.js · TypeScript · Tailwind CSS
Cloud      │ AWS · GCP · Docker · Railway · Vercel · Firebase
```

---

## Currently

- 🔨 Building: Advanced RAG pipelines and multi-agent systems
- 📚 Learning: LangGraph · Advanced RAG · LLM fine-tuning
- 💼 Open to: **AI Engineer · LLM Engineer · AI Product Engineer** (3–5 yrs) — US-based, remote/hybrid
- 📍 United States
